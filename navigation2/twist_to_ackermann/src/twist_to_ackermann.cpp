...
: Node("twist_to_ackermann", options)
{
  ...
  if (_use_stamps) {
    ...
    _acks_pub = this->create_publisher<ackermann_msgs::msg::AckermannDriveStamped>("/ackermann_cmd", 10);
  } else {
    _twist_sub = this->create_subscription<geometry_msgs::msg::Twist>(
      "/cmd_vel", 10, std::bind(&TwistToAckermann::twist_cb, this, std::placeholders::_1));

    _ack_pub = this->create_publisher<ackermann_msgs::msg::AckermannDrive>("/ackermann_cmd", 10);
  }
